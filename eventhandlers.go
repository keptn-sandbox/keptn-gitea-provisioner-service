...
// HandleGetSliTriggeredEvent handles get-sli.triggered events if SLIProvider == keptn-gitea-provisioner-service
...
func HandleGetSliTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.GetSLITriggeredEventData) error {
	...
	if data.GetSLI.SLIProvider != "keptn-gitea-provisioner-service" {
	...
	// Get SLI File from keptn-gitea-provisioner-service subdirectory of the config repo - to add the file use:
	//   keptn add-resource --project=PROJECT --stage=STAGE --service=SERVICE --resource=my-sli-config.yaml  --resourceUri=keptn-gitea-provisioner-service/sli.yaml
	sliFile := "keptn-gitea-provisioner-service/sli.yaml"
